...
def recive():
    cl = conn.recv(BUFFER_SIZE).decode(encoding='utf_8', errors='strict')
    decoding = cl.replace("1010 ", " ")
...
def send():
    print(conn)
    sv = str(input("Type:")).lower()
    # - = 0 . = 1 1010 = " "
    encoding = sv.replace(" ", "1010 ")
    ...
    sv = str(encryptedmessage)
    conn.send(sv.encode(encoding='utf_8', errors='strict'))

while True:
    if __name__ == '__main__':
        p1 = Process(target=send)
        p2 = Process(target=recive)
        p1.start()
        p2.start()
        p1.join()
        p2.join()
